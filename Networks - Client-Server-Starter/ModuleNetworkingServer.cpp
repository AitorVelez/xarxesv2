...
bool ModuleNetworkingServer::start(int port)
{
	...
	// - Set address reuse
	int enable = 1;
	int val = setsockopt(listenSocket, SOL_SOCKET, SO_REUSEADDR, (const char*)&enable, sizeof(enable));
	if (val == SOCKET_ERROR)
	{
	...
	val = bind(listenSocket, (const sockaddr*)&bindAddr, sizeof(bindAddr));
	if (val == SOCKET_ERROR)
	{
		reportError("bind");
		return false;
	...
	val = listen(listenSocket, connections);
	if (val == SOCKET_ERROR)
	{
		reportError("listen");
		return false;
